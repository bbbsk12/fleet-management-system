...
"""
车队管理系统 Web 后端服务器 — ROS2 集成版

集成了 ROS2 通信，对接真实底盘和 fleet_manager 调度系统。

环境变量:
    FLEET_STATUS_TOPIC: 车队状态话题名称，默认 /fleet_manager/fleet_status_traffic
                        （含交通定位字段）。若无 fleet_manager 可设为 /fleet_monitor/fleet_status。
    FLEET_TRAFFIC_MAP_FILE: 交通地图文件路径
    FLEET_WEB_SETTINGS_FILE: WebUI 设置文件路径

功能说明：
    - 通过 ROS2 订阅获取实时机器人状态和任务状态
    - 提供 REST API 和 WebSocket 接口供前端使用
    - 支持地图加载、航点管理、任务调度
    - 支持机器人召回、移除出队等管理操作
"""
...
class SettingsModel(BaseModel):
    ...
    属性:
        ros_domain_id: ROS_DOMAIN_ID 值
        zenoh_router: Zenoh 路由器地址
        ws_port: WebSocket 端口号
    """
...
class TaskInfo(BaseModel):
    ...
    属性:
        ...
        task_type: 任务类型（1=CRUISE, 2=LOAD, 3=UNLOAD, 4=SITE_SPECIFIC）
        site_code: 业务站点代码
        created_at: 创建时间
    """
    id: str
...
class ROSBridgeNode(Node if HAS_ROS else object):
    """ROS2 桥接节点，负责与 ROS2 系统通信。

    主要职责：
        - 订阅车队状态话题，实时更新机器人状态
        - 订阅任务状态话题，实时更新任务进度
        - 提供任务提交、取消、移除机器人的 ROS2 服务客户端
    """

    def __init__(self, state_manager):
        """初始化 ROS2 桥接节点。

        创建所有必要的订阅者和服务客户端，连接到 fleet_manager 节点。

        参数:
            state_manager: FleetState 全局状态管理器实例
        """
        if HAS_ROS:
    ...
    def task_status_callback(self, msg):
        ...
        接收 fleet_manager 发布的任务状态更新，同步更新本地状态，
        并通过 WebSocket 广播给前端。

        参数:
            msg: TaskInfoMsg 类型的 ROS2 消息，包含任务 ID、状态、分配机器人等信息
        """
        task_id = msg.task_id
        status = msg.status
    ...
    def fleet_callback(self, msg: FleetStatus):
        ...
        接收 fleet_manager 发布的完整车队状态，处理世界坐标到像素坐标的转换，
        合并新旧机器人状态，并通过 WebSocket 广播给前端。

        参数:
            msg: FleetStatus 类型的 ROS2 消息，包含所有机器人的实时状态
        """
        new_robots = {}
...
class FleetState:
    ...
    管理机器人状态、任务列表、航点数据、地图信息、系统日志、
    WebSocket 客户端连接以及 ROS2 桥接节点引用。
    ...
    def __init__(self):
        ...
        self.removed_robots: set = set()
        self.tasks: Dict[str, TaskInfo] = {}
        self.waypoints: List[Dict] = []
        self.map_data: Dict = {}
        self.map_dir: str = ""
        self.map_resolution: float = 0.05
        self.map_origin: List[float] = [0.0, 0.0, 0.0]
    ...
    def world_to_pixel(self, world_x: float, world_y: float) -> tuple:
        ...
        ROS 地图 origin 是图像左下角在世界坐标系中的位置。
        图像坐标系 Y 轴向下，世界坐标系 Y 轴向上，需要翻转。
...
def load_traffic_map(map_path: str = None):
    ...
    从 YAML 格式的地图文件中读取航点列表、地图分辨率、原点信息，
    并获取地图图片高度用于像素坐标转换。

    参数:
        map_path: 地图文件路径。为 None 时自动在仓库 maps/ 目录下查找。
    """
    if map_path is None:
...
async def lifespan(app: FastAPI):
    ...
    启动时按顺序执行：
        1. 获取事件循环引用
        2. 加载持久化设置（在 rclpy.init 之前设置 ROS_DOMAIN_ID）
        3. 加载交通地图
        4. 初始化 ROS2 节点并启动后台 spin 线程

    关闭时执行：
        - 销毁 ROS2 节点并调用 rclpy.shutdown()
    """
    fleet_state.event_loop = asyncio.get_event_loop()

    # ---- 加载持久化设置（用于 ROS_DOMAIN_ID 等环境变量） ----
    s = load_settings()
...
async def update_settings(req: SettingsModel):
    ...
    部分设置项（如 ros_domain_id、zenoh_router）需要重启服务方可生效。
...
async def get_map_image():
    """获取地图图片（返回 base64 编码格式）。

    PGM 格式图片自动转换为 PNG，以兼容浏览器显示。
...
async def create_task(request: SubmitTaskRequest):
    ...
    如果未指定 robot_id，则由 fleet_manager 自动选择空闲底盘执行；
    这避免了因 nav_status 采样延迟而误判繁忙机器人为空闲的问题。

    参数:
        request: 包含航点 ID、优先级、机器人 ID、任务类型和站点代码的请求体
...
async def assign_task(task_id: str, request: AssignTaskRequest):
    ...
    仅支持状态为 pending 或 assigned 的任务进行重新分配。

    参数:
        task_id: 任务 ID
        request: 包含目标机器人 ID 的请求体
...
async def cancel_task(task_id: str):
    ...
    优先调用 ROS2 侧的取消服务（真正取消正在执行的 Nav2 goal），
    若 ROS2 服务不可用则仅标记本地状态（不保证底盘停止）。
...
async def emergency_stop(robot_id: Optional[str] = None):
    ...
    通过 WebSocket 广播紧急停止指令给前端，由前端展示告警。
...
async def get_status():
    ...
    返回 ROS2 连接状态、在线机器人数、任务统计和航点数量等概览数据。
    """
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    支持的消息类型：
        ...
        - emergency_stop: 紧急停止
        - recall_robot: 召回机器人
        - remove_robot: 移除机器人出队

    连接建立后自动发送初始状态（含机器人列表、任务列表、航点、地图数据和 ROS2 连接状态）。
    """
